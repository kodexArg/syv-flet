...
description: Flet implementation for hexagonal grid rendering, scrolling, and click detection. Covers canvas methods, coordinate transforms, and user input handling.
allowed-tools: Read, Grep, Write, Edit, Bash
---

# Hexagonal Grid Flet Rendering

Technical guide for rendering SyV-Flet's hexagonal grid on Flet canvas, implementing scroll/zoom, and detecting user clicks.

## Scope

This skill covers **grid rendering and interaction only**:
- Drawing hexagons on Flet canvas
- Converting pixel clicks to hex coordinates
- Scrolling and zooming the grid
- Rendering order overlays (icons, outlines)

For complete UI/UX interface design (buttons, layouts, phases), see `ux-ui-flet-rendering`.
For mathematical foundations, see `hex-grid-math`.
...
## Hex Tap Cycling: Visual Rendering

> **Interaction Logic:** See `cycle-tap-mechanism` skill for complete explanation of tap cycling states, order types, and state transitions.

Orders are placed by **clicking hexagon centers**. This section covers the **rendering implementation** of visual feedback during tap cycling.

### Click-to-Hex Conversion (Recap)

See earlier section "Interaction: Click → Hex Conversion" for detailed pixel-to-hex mapping.
...
def on_tap(self, e: ft.TapEvent) -> tuple[int, int]:
    ...
    click_x = e.local_x
    click_y = e.local_y
    q, r = self.renderer.pixel_to_hex_rounded(click_x, click_y, hex_size)
...
### Rendering Order Highlights

#### Origin Hex Highlight (Double Outline)

```python
def highlight_origin_hex(self, q: int, r: int, hex_size: float, faction_color: str):
    """
    Draw double outline on origin hex (selected for order placement).
    Faction color indicates which player selected it.
    """
    col, row = self.axial_to_offset(q, r)
    cx, cy = self.offset_to_pixel_center(col, row, hex_size)

    # Outer outline (thick, faction color)
    outer_outline = cv.Path(
        data=self.create_hex_path_data(cx, cy, hex_size),
        stroke=faction_color,
        stroke_width=3,
        fill="none"
    )

    # Inner highlight (subtle fill)
    inner_fill = cv.Path(
        data=self.create_hex_path_data(cx, cy, hex_size * 0.95),
        stroke="none",
        fill=faction_color,
        fill_opacity=0.2
    )

    return [outer_outline, inner_fill]
```

#### Adjacent Hex Highlighting (Hover Feedback)

```python
def highlight_adjacent_hex(self, q: int, r: int, hex_size: float, is_hover: bool = False):
    """
    Draw single outline on adjacent hex.
    Opacity varies based on hover state and order presence.
    """
    col, row = self.axial_to_offset(q, r)
    cx, cy = self.offset_to_pixel_center(col, row, hex_size)

    # Opacity: 0.4 idle, 0.7 hover
    stroke_width = 2 if is_hover else 1
    opacity = 0.7 if is_hover else 0.4

    adjacent_outline = cv.Path(
        data=self.create_hex_path_data(cx, cy, hex_size),
        stroke="#CCCCCC",  # Neutral gray
        stroke_width=stroke_width,
        fill="none",
        opacity=opacity
    )

    return adjacent_outline
```

#### Order Icon & Label (Destination Hex)

```python
def draw_order_icon(self, q: int, r: int, order_type: str, hex_size: float,
                    faction_color: str, visibility_opacity: float = 1.0):
    """
    Draw order type icon on hex (after tap cycling selects it).

    Args:
        visibility_opacity: 0.4 (PLANIFICACIÓN, secret) or 1.0 (EJECUCIÓN, public)
    """
    col, row = self.axial_to_offset(q, r)
    cx, cy = self.offset_to_pixel_center(col, row, hex_size)

    # Load icon file
    icon_file = f"assets/icons/{order_type.lower()}.png"

    # Center icon on hex
    icon_size = hex_size * 0.6
    ...
    # Stack icon + label centered on hex
    return ft.Stack([
        ft.Positioned(
            left=icon_x,
            top=icon_y,
...
> See `hexagonal-grid-ui-ux` skill section "Order Visibility: Opacity and Phase Transitions" for visual design specifications.

**Key Rendering Rule:**
- All hex highlighting code is **opacity-aware**
- Pass `visibility_opacity` parameter based on game phase:
  - PLANIFICACIÓN: `visibility_opacity=0.4` (orders are secret)
  - EJECUCIÓN: `visibility_opacity=1.0` (all orders visible)
...
**Order Visualization:**
- [ ] Origin hex double outline renders correctly (faction color, 3px stroke)
- [ ] Adjacent hex single outline with correct opacity (gray, 1-2px stroke)
- [ ] Order icons render at hex center with correct size (60% of hex)
- [ ] Order labels visible and readable (white, bold, 10px size)
- [ ] Icons load correctly from `assets/icons/PNG/`

**Phase Transitions:**
- [ ] Opacity values correct (0.4 PLANIFICACIÓN, 1.0 EJECUCIÓN)
- [ ] Orders fade out when switching players (Cambiar Jugador click)
- [ ] Orders fade in when entering EJECUCIÓN phase

**Performance:**
- [ ] Canvas render: <5ms for grid
- [ ] Dynamic updates: <5ms for order icons
- [ ] Sustained 60 FPS on desktop, 30 FPS on mobile
